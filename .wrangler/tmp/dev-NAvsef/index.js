...
// .wrangler/tmp/bundle-fQ5DSq/checked-fetch.js
...
// .wrangler/tmp/bundle-fQ5DSq/strip-cf-connecting-ip-header.js
        ...
        <div class="selector-row">
          <div class="selector-group">
            ...
            <select id="serverSelect" class="custom-select">
              <option value="pahe">Pahe Server (Proxied M3U8)</option>
  ...
  <script>
    ...
    async function playEpisode(episodeNum) {
      ...
      try {
        ...
        if (provider === 'pahe') {
          // Fetch from stream source proxy
          const res = await fetch(\`/api/streams?slug=\${activeAnime.slug}&episode=\${episodeNum}&provider=pahe&lang=\${lang}\`);
          const json = await res.json();
          
          if (!json.success || !json.data || !json.data.sources || json.data.sources.length === 0) {
            throw new Error(json.error || "No streaming sources returned from Pahe.");
          }
            ...
            if (data.fatal) {
              ...
              switch (data.type) {
                case Hls.ErrorTypes.NETWORK_ERROR:
                  log("Fatal network error! Attempting to recover...", 'info');
                  hls.startLoad();
                  break;
                case Hls.ErrorTypes.MEDIA_ERROR:
                  log("Fatal media error! Attempting to recover...", 'info');
...
app.all("/stream/*", async (c) => {
  ...
  const targetUrl = `https://prox.anikage.cc${path}`;
  const headers = new Headers();
  headers.set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36");
  headers.set("Referer", "https://anikage.cc/");
  ...
  try {
    ...
    if (path.includes("index.txt") || path.endsWith(".m3u8") || contentType.includes("mpegurl")) {
      let text = await res.text();
      text = text.replaceAll("/stream/", "https://prox.anikage.cc/stream/");
...
app.get("/api/streams", async (c) => {
  ...
  const streams = await getStreams(slug, episode, provider, lang);
  const origin = new URL(c.req.url).origin;
  if (streams && Array.isArray(streams.sources)) {
    streams.sources = streams.sources.map((src) => {
      if (src.streamUrl && src.streamUrl.startsWith("https://prox.anikage.cc")) {
        src.streamUrl = src.streamUrl.replace("https://prox.anikage.cc", origin);
      }
      return src;
    });
  }
...
// .wrangler/tmp/bundle-fQ5DSq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fQ5DSq/middleware-loader.entry.ts
